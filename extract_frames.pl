...
sub getOutputFPS {
    ...
    $command = 'avprobe '.$file.' 2>&1 | grep -e \'Video\' | awk -F \' \' \'{print $12}\'';
    my $fps = system($command);
...
sub makeOutputDirectory {
    ...
    system($input);
    if($? eq 0) {
avprobe /media/jelevoir/FilesBackups1.5TB/Anime/Anime\ Shows/Dragonball\!/Season\ 3/Dragonball\ 092\ -\ Goku\ Enters\ the\ Ring.mkv 2>&1 | grep -e 'Video'        print "Directory created or exists!\n";
        return $filepath;
    } else {
        die "There was a problem creating $filepath.\nError code: $?";
    }
